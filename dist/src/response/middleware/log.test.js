...
const mockRequest = Object.assign({}, getRequestFixture);
const mockResponse = {
    body: 'foobar',
    headers: {
        foo: 'bar',
...
describe('Request/Response logging ', () => {
    ...
    test('is logged by custom options.logger if one is provided', () => {
        ...
        const response = log(mockResponse, mockRequest, Object.assign({}, mockOptions, { logger: mockCustomLogger }));
...
describe('Default logger', () => {
    test('should construct a requestUri when request contained query string parameters', () => {
        ...
        jest.spyOn(process.stdout, 'write').mockImplementationOnce((_) => true);
        expect(logger(Object.assign({}, mockRequest, { query: { foo: 'bar', bar: 'foo' } }), mockResponse).requestUri).toBe('/?foo=bar&bar=foo');
    });
    ...
    test('should write to process.stderr when statusCode is in the 500s', () => {
        const spy = jest
            ...
            .mockImplementationOnce(item => !expect(item).toContain('statusCode'));
        const result = logger(mockRequest, Object.assign({}, mockResponse, { statusCode: 500 }));
        expect(result.statusCode).toBe(500);
        expect(spy).toHaveBeenCalledTimes(1);
    });
    test('should include httpProtocol', () => {
        jest.spyOn(process.stdout, 'write').mockImplementationOnce((_) => true);
        const result = logger(Object.assign({}, mockRequest, { headers: Object.assign({}, mockRequest.headers, { via: 'foo bar' }) }), mockResponse);
        expect(result.httpProtocol).toBe('foo');
    });
    test('should include request content-length', () => {
        jest.spyOn(process.stdout, 'write').mockImplementationOnce((_) => true);
        expect(logger(Object.assign({}, mockRequest, { body: 'foobar' }), mockResponse).requestContentLength).toBe(6);
        jest.spyOn(process.stdout, 'write').mockImplementationOnce((_) => true);
        expect(logger(Object.assign({}, mockRequest, { body: undefined }), mockResponse).requestContentLength).toBe(0);
    });
});
