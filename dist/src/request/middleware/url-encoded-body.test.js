...
const testRequest = Object.assign({}, getRequestFixture, { body: 'foo=bar&bar=foo', headers: {
...
describe('Request urlEncoded body', () => {
    ...
    test('body is not parsed if content-type is not application/x-www-form-urlencoded', () => {
        const invalidTestRequest = Object.assign({}, testRequest, { headers: { 'content-type': 'foo/bar' } });
    ...
    test('correctly handle shitty body', () => {
        const expected = Object.assign({}, testRequest, { body: -1 });
        const result = parseUrlEncodedBody(Object.assign({}, testRequest, { body: -1 }));
        expect(result).toEqual(expected);
    });
});
